  ...
  return (
    <div style={{ minHeight: '100vh', fontFamily: "'Outfit', system-ui, sans-serif", color: '#dff0e1', background: '#010d03', position: 'relative' }}>
      ...
      <main style={{ position: 'relative', zIndex: 10, maxWidth: 1200, margin: '0 auto', padding: '0 28px 80px' }}>
        ...
        {/* DIVIDER */}
        <div style={{ width: '100%', height: 1, background: 'linear-gradient(90deg,transparent,rgba(13,153,96,.15) 30%,rgba(200,148,26,.14) 70%,transparent)', margin: '0 0 52px' }} />

        {/* STATS ROW */}
        <div className="fi3" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 56 }}>
          {[
            { value: '95',    label: 'HIV diagnoses in NZ', note: 'New cases · 2024',             color: 'rgba(248,110,110,.9)' },
            { value: '2,312', label: 'People on ART',       note: 'Health NZ · NZAF 2024',        color: 'rgba(30,220,130,.9)' },
            { value: '2030',  label: 'Zero target',         note: 'Te Tiriti · end transmissions', color: 'rgba(240,188,56,.9)' },
            { value: '35',    label: 'Topics tracked',      note: '6 categories · 140 signals',    color: 'rgba(30,220,130,.9)' },
          ].map(({ value, label, note, color }) => (
            <div key={label} className="sp-lift" style={{ ...SP.glass, borderRadius: 18, padding: '22px 18px', textAlign: 'center' }}>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(30px,3.2vw,46px)', fontWeight: 300, color, letterSpacing: '-.02em', lineHeight: 1.1 }}>{value}</div>
              <div style={{ fontSize: 12.5, color: '#dff0e1', fontWeight: 500, margin: '8px 0 4px', lineHeight: 1.3 }}>{label}</div>
              <div style={{ fontSize: 10.5, color: 'rgba(223,240,225,.35)', letterSpacing: '.04em' }}>{note}</div>
            </div>
          ))}
        </div>
