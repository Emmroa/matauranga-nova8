// ═══════════════════════════════════════════════════════════════════════════
// Dashboard.jsx — NOVA Admin Dashboard — 5-tab analytics & ops centre
...
// ─── Chart hook: create once on mount, destroy on unmount ──────────────────
      ...
      datasets: [
        {
          ...
          borderColor: 'rgba(200,148,26,.9)',
          backgroundColor: 'rgba(200,148,26,.08)',
          fill: false,
          tension: 0.4,
          pointRadius: 3,
          yAxisID: 'y',
        },
        {
          ...
          borderColor: 'rgba(248,110,110,.8)',
          backgroundColor: 'rgba(248,110,110,.06)',
          fill: false,
          tension: 0.4,
          pointRadius: 3,
          yAxisID: 'y1',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr', gap: 16 }}>
        ...
        <div style={{ ...C.cardGold, padding: '20px 18px' }}>
          ...
          <div style={{ fontSize: 12, color: 'rgba(248,110,110,.65)', lineHeight: 1.65, marginTop: 8, padding: '12px 14px', borderRadius: 11, background: 'rgba(248,110,110,.05)', border: '1px solid rgba(248,110,110,.14)' }}>
            ◉ Live data feed pending connection. Deploy to production to activate crisis signal tracking.
          </div>
          <div style={{ marginTop: 12, fontSize: 11, color: 'rgba(223,240,225,.35)', lineHeight: 1.6 }}>
            Each activation = detected crisis phrase (suicide ideation, acute distress). User shown 111, Lifeline 0800 543 354, 1737 immediately.
          </div>
        </div>
      </div>
    </div>
...
const ZONES = [
  {
    ...
    hiv: 38, art: 920, sdhi: 6.2, gap: 5.1, outbreak: 12,
    warn: null,
    indices: [
  ...
  },
  {
    ...
    hiv: 22, art: 510, sdhi: 7.1, gap: 6.8, outbreak: 34,
  ...
  },
  {
    ...
    hiv: 18, art: 440, sdhi: 5.9, gap: 4.8, outbreak: 22,
    warn: null,
    indices: [
  ...
  },
  {
    ...
    hiv: 17, art: 440, sdhi: 7.8, gap: 7.4, outbreak: 41,
  ...
  return (
    <div style={{ ...C.card, overflow: 'hidden', transition: 'border-color .2s', ...(open ? { borderColor: 'rgba(200,148,26,.3)' } : {}) }}>
      <button onClick={() => setOpen(o => !o)}
        ...
        {zone.warn && (
          <div style={{ ...warnStyle, fontSize: 11, padding: '7px 11px', borderRadius: 9, fontWeight: 500 }}>
            ◉ {zone.warn.text}
          </div>
        )}
      </button>

      <div style={{ maxHeight: open ? `${contentRef.current?.scrollHeight || 500}px` : '0px', overflow: 'hidden', transition: 'max-height 0.32s cubic-bezier(0.4,0,0.2,1)' }}>
  ...
  return (
    <div className="db-fade">
      <div style={{ fontSize: 12, color: 'rgba(223,240,225,.38)', marginBottom: 18, padding: '10px 14px', background: 'rgba(200,148,26,.04)', border: '1px solid rgba(200,148,26,.12)', borderRadius: 10 }}>
        Click a zone to expand details · SDHI = Social Determinants Health Index · Scale 1–10 (higher = greater disadvantage)
      </div>
...
function TabPredictive() {
  ...
  useChart(projRef, (el) => new Chart(el, {
    ...
    data: {
      ...
      datasets: [
        {
          label: 'NZ total (actual)',
          data: [95, 92, 88, 91, 87, 89, null, null],
          borderColor: 'rgba(30,220,130,.8)',
          backgroundColor: 'rgba(30,220,130,.06)',
          fill: false, tension: 0.3, pointRadius: 3,
        },
        {
          label: 'Forecast (model)',
          data: [null, null, null, null, null, 89, 93, 98],
          borderColor: 'rgba(200,148,26,.7)',
          borderDash: [5, 3],
          fill: false, tension: 0.3, pointRadius: 3,
        },
        {
          label: 'Southern risk proxy',
          data: [22, 24, 21, 26, 23, 28, 31, 35],
          borderColor: 'rgba(248,110,110,.7)',
          backgroundColor: 'rgba(248,110,110,.05)',
...
function TabStatus() {
  ...
  const SERVERS = [
    ...
    { name: 'Vite dev :5173',         sub: 'Production: served via Nginx',    icon: '🔧', color: 'rgba(200,148,26,.7)' },
    { name: `phi3:mini Ollama`,       sub: health ? (health.ollamaUp ? (health.modelLoaded ? 'Loaded · KV cache warm' : 'Running · loading') : 'Not responding') : 'Checking…', icon: health?.modelLoaded ? '⚠️' : '⚠️', color: 'rgba(200,148,26,.8)' },
      ...
      <header style={{ background: 'linear-gradient(180deg,rgba(2,18,8,.92) 0%,rgba(1,13,5,.78) 100%)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', borderBottom: '1px solid rgba(13,153,96,.14)', position: 'sticky', top: 0, zIndex: 30 }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 54 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 14, fontWeight: 300 }}>Mātauranga <span style={{ color: 'rgba(200,148,26,.88)' }}>NOVA</span> <span style={{ color: 'rgba(223,240,225,.4)', fontSize: 12 }}>· Analytics</span></div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Link to="/" style={{ fontSize: 11, color: 'rgba(223,240,225,.38)', textDecoration: 'none', padding: '5px 12px', border: '1px solid rgba(255,255,255,.07)', borderRadius: 7, transition: 'color .2s' }}>← Home</Link>
